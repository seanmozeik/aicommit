import type { ClassifiedFiles, FileDiff, SemanticInfo } from './types';
...
export const extractSemantics = (files: FileDiff[]): SemanticInfo => {
  ...
  const functions = [
    ...allAddedCode.matchAll(/(?:function|async function)\s+(\w+)\s*\(/gu),
    ...allAddedCode.matchAll(/(?:const|let|var)\s+(\w+)\s*=\s*(?:async\s*)?\(/gu),
    ...allAddedCode.matchAll(/(?:const|let|var)\s+(\w+)\s*=\s*(?:async\s*)?\w*\s*=>/gu),
  ].map((m) => m[1]);

  const types = [...allAddedCode.matchAll(/(?:interface|type)\s+(\w+)/gu)].map((m) => m[1]);

  const classes = [...allAddedCode.matchAll(/class\s+(\w+)/gu)].map((m) => m[1]);

  const exports = [
    ...allAddedCode.matchAll(
      /export\s+(?:default\s+)?(?:function|const|class|interface|type|async function)\s+(\w+)/gu,
    ),
  ].map((m) => m[1]);
